...
sityva = input("შემოიყვანეთ ტექსტი")
sityva_reverse = sityva[::-1]

if sityva == sityva_reverse:
    print("შემოყვანილი ტექსტი პალინდრომია")

else:
    vipovet = False
    for i in range(len(sityva_reverse)):
        testi = sityva[:i] + sityva[i+1:]
        if  testi == (testi)[::-1]:
            print(f"გთავაზობთ: {testi}")
            vipovet = True
            break
        
    if vipovet == False:    
        for i in range(len(sityva)+1):
            for k in sityva:
                satesto = sityva[:i] + k + sityva[i:]
                if satesto == satesto[::-1]:
                    print(f"გთავაზობთ: {satesto}")
                    vipovet = True
                    break
            if vipovet == True:
                break
